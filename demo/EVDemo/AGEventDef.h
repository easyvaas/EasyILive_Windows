...
{
	unsigned int	uid;
	EV_USER_OFFLINE_REASON_TYPE		reason;
...
typedef struct _EV_CREATE_CHANNEL_RES
{
	char push_url[MAX_PATH];  //旁路推流地址
	char channel_id[MAX_PATH]; //频道id
}EV_CREATE_CHANNEL_RES, *PEV_CREATE_CHANNEL_RES, *LPEV_CREATE_CHANNEL_RES;

#define EID_EV_CREATE_CHANNEL                   0x00000021

//用户鉴权返回数据
typedef struct _EV_USER_AUTH_RES
{
	char auth_key[MAX_PATH];    //鉴权key，SetChannelRole时使用，用于鉴权
	unsigned int owner_id;      // 频道创建者uid
	char channel_key[MAX_PATH]; // JoinChannel传入
	unsigned int uid;           // uid

}EV_USER_AUTH_RES, *PEV_USER_AUTH_RES, *LPEV_USER_AUTH_RES;

#define EID_EV_BAD_NETWORK                       0x00000025
#define EID_EV_COMM_HEART_BEAT                   0x00000026

#define EID_WINDOWS_SHARE_START                  0x00000050
#define EID_WINDOWS_SHARE_STOP                   0x00000051
#define EID_SCREEN_CAPTURE_DESKTOP               0x00000052 //显示器截屏
#define EID_CHANED_VIDEOPROFILE                  0x00000059
#define EID_START_LOCAL_PREVIEW                  0x00000055 //开启本地预览
#define WM_MSG_SHAREWINDOW_SIZE 0x00000056
#endif