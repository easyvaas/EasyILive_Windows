...
void CAGVideoWnd::SetWndIndex(int index)
{
	m_nIndex = index;
}

int CAGVideoWnd::GetWndIndex()
{
	return m_nIndex;
}
...
int CAGVideoWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// TODO:  在此添加您专用的创建代码;
	m_wndInfo.Create(NULL, NULL, WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, IDC_STATIC);
...
void CAGVideoWnd::SetBigShowFlag(BOOL bBigShow)
{
	...
	if (m_wndInfo.GetSafeHwnd() != NULL) {
		...
		m_wndInfo.MoveWindow(x, y, 0, 0); // 192, 24);
	}
};
...
{
	::SendMessage(GetParent()->GetSafeHwnd(), WM_ADDTOLIVE, (WPARAM)this, (LPARAM)m_nIndex);
